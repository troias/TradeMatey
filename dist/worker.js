...
const HUBSPOT_ACCESS_TOKEN = process.env.HUBSPOT_ACCESS_TOKEN ||
    ...
    "";
const USE_OAUTH = Boolean(process.env.HUBSPOT_CLIENT_ID && process.env.HUBSPOT_CLIENT_SECRET);
...
async function refreshTokenIfNeeded(portal) {
    ...
    for (const row of data) {
        try {
            ...
            try {
                ...
                const contactRes = await upsertContact(portal, String(user.email), (user.roles || []));
                ...
                // record metrics
                const { error: metricErr } = await supabase.rpc("upsert_hubspot_worker_metric", {
                ...
                if (attempt >= MAX_ATTEMPTS) {
                    ...
                    // inc dlq metric
                    const { error: metricErr2 } = await supabase.rpc("upsert_hubspot_worker_metric", {
                        p_portal_id: portal.portal_id,
                        p_processed: 0,
                        p_errors: 0,
                        p_dlq: 1,
...
async function upsertContact(portal, email, roles) {
    // resolve token order:
    ...
    if (portal.encrypted_access_token && APP_KEY) {
        const dec = decrypt(portal.encrypted_access_token, APP_KEY);
        token = dec ?? token;
    }
    ...
    if (res.status === 401) {
        if (USE_OAUTH) {
            ...
            if (portal.encrypted_access_token && APP_KEY) {
                const d = decrypt(portal.encrypted_access_token, APP_KEY);
    ...
    const found = j && Array.isArray(j.results) && j.results.length > 0 ? j.results[0] : null;
    const props = { email };
    if (roles && roles.length)
        props["app_roles"] = roles; // assumes multi-select property 'app_roles' exists
    ...
    try {
        ...
        if (!sErr && settings && settings.property_mappings) {
            const map = settings.property_mappings;
        ...
        console.warn("mapping lookup failed", err instanceof Error ? err.message : String(err));
    }
    if (!found) {
        // create
